...
## Overview
Adds production-grade group messaging infrastructure:
- Group metadata fields (description, category, privacy, cover image) on conversations
- Role-based access control on conversation_participants (owner/admin/moderator/member/read_only)
- Pinned messages table (admin/owner controlled)
- Group join requests table (for invite-only / private groups)
- Auto-assign 'owner' role to conversation creator via trigger

## Security
- RLS on all new tables.
- pinned_messages: participants can view; only admins/owners can insert/delete.
- group_join_requests: users create own; admins view/approve/reject.
*/
