...
## Overview
Upgrades the feed from basic text+image posts to a full social content system:
- Rich posts with multiple media, polls, code snippets, links, and visibility controls
- Multi-reaction system (like, love, celebrate, insightful, funny, confused)
- Nested comments with replies, pinning, and sorting
- Saved posts with user-created collections
- Post drafts with autosave support

## Security
- RLS enabled on all new tables.
- Reactions: any authenticated user can react to visible posts; users can delete own reactions.
- Comment likes: any authenticated user can like/unlike comments.
- Saved posts: users can only see/manage own saved posts.
- Collections: users can only see/manage own collections.
- Drafts: users can only see/manage own drafts.
*/
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='posts' AND column_name='poll_question') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='posts' AND column_name='poll_expires_at') THEN
    ALTER TABLE posts ADD COLUMN poll_expires_at timestamptz;
  END IF;
END $$;

DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='posts' AND column_name='code_language') THEN
    ALTER TABLE posts ADD COLUMN code_language text;
  END IF;
END $$;
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='posts' AND column_name='link_title') THEN
    ALTER TABLE posts ADD COLUMN link_title text;
  END IF;
END $$;

DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='posts' AND column_name='link_description') THEN
    ALTER TABLE posts ADD COLUMN link_description text;
  END IF;
END $$;

DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='posts' AND column_name='link_image') THEN
    ALTER TABLE posts ADD COLUMN link_image text;
  END IF;
END $$;
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='comments' AND column_name='pinned') THEN
...
CREATE POLICY "insert_comment_likes" ON comment_likes FOR INSERT
  TO authenticated WITH CHECK (user_id = auth.uid());

DROP POLICY IF EXISTS "delete_comment_likes" ON comment_likes;
CREATE POLICY "delete_comment_likes" ON comment_likes FOR DELETE
  TO authenticated USING (user_id = auth.uid());

-- 5. collections table (MUST come before saved_posts)
...
CREATE POLICY "insert_collections" ON collections FOR INSERT
  TO authenticated WITH CHECK (user_id = auth.uid());
...
CREATE POLICY "delete_collections" ON collections FOR DELETE
  TO authenticated USING (user_id = auth.uid());

-- 6. saved_posts table (references collections)
...
CREATE POLICY "select_saved_posts" ON saved_posts FOR SELECT
  TO authenticated USING (user_id = auth.uid());

DROP POLICY IF EXISTS "insert_saved_posts" ON saved_posts;
CREATE POLICY "insert_saved_posts" ON saved_posts FOR INSERT
  TO authenticated WITH CHECK (user_id = auth.uid());

DROP POLICY IF EXISTS "delete_saved_posts" ON saved_posts;
CREATE POLICY "delete_saved_posts" ON saved_posts FOR DELETE
  TO authenticated USING (user_id = auth.uid());

DROP POLICY IF EXISTS "update_saved_posts" ON saved_posts;
CREATE POLICY "update_saved_posts" ON saved_posts FOR UPDATE
  TO authenticated USING (user_id = auth.uid()) WITH CHECK (user_id = auth.uid());
...
CREATE POLICY "select_drafts" ON post_drafts FOR SELECT
  TO authenticated USING (user_id = auth.uid());

DROP POLICY IF EXISTS "insert_drafts" ON post_drafts;
CREATE POLICY "insert_drafts" ON post_drafts FOR INSERT
  TO authenticated WITH CHECK (user_id = auth.uid());

DROP POLICY IF EXISTS "update_drafts" ON post_drafts;
CREATE POLICY "update_drafts" ON post_drafts FOR UPDATE
  TO authenticated USING (user_id = auth.uid()) WITH CHECK (user_id = auth.uid());

DROP POLICY IF EXISTS "delete_drafts" ON post_drafts;
CREATE POLICY "delete_drafts" ON post_drafts FOR DELETE
  TO authenticated USING (user_id = auth.uid());
