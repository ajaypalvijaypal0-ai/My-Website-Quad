...
This migration extends the existing Quad schema to support production-grade
real-time messaging, online presence, typing indicators, read receipts,
group chat metadata, pinned conversations, and a per-user editable timetable.

## 1. Profiles — Presence
- Added `is_online` (boolean, default false) and `last_seen` (timestamptz).

## 2. Conversations — Group Chat
- Added `is_group`, `group_name`, `group_avatar`, `created_by`.

## 3. Conversation Participants — Pin & Admin
- Added `pinned` (boolean) and `is_admin` (boolean).

## 4. Messages — Rich Media
- Added `type`, `media_url`, `edited_at`.

## 5. New Table: message_status
- Per-user read/delivered status for each message.

## 6. New Table: typing_status
- Tracks who is typing in which conversation.

## 7. New Table: timetable
- Per-user class schedule entries with RLS.

## 8. Storage
- Creates `chat-media` bucket.

## 9. Realtime
- Enables realtime on all new and modified tables.

## 10. RLS Policies
- Full CRUD policies for all new tables and updated existing tables.
*/
...
-- conversation_participants: update own + delete own
